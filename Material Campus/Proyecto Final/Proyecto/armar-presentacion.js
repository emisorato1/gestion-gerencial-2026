// Presentacion Dulxelitos, 8 laminas.
// El contenido sale del pptx que corrigio el usuario; lo unico rearmado es el
// canvas, que ahora agrupa los nueve bloques en cuatro preguntas.
...
pres.author = "Lepez Joaquin, Geyer Juan Jose, Lopez Juan, Sorato Emiliano, Olivares Manuel";
...
function titulo(s, texto, color) {
  s.addText(texto, {
    x: M, y: 0.5, w: CW, h: 0.62, isTextBox: true, margin: 0,
...
function bajada(s, texto, color) {
  s.addText(texto, {
    x: M, y: 1.14, w: CW, h: 0.38, isTextBox: true, margin: 0,
...
function pie(s, color) {
  s.addText("Dulxelitos · Gestión Gerencial · UTN FRSR", {
    x: M, y: 6.85, w: CW, h: 0.3, isTextBox: true, margin: 0,
    fontFace: B, fontSize: 10, color: color || MUTED, align: "right", valign: "middle",
  });
}
...
function tarjeta(s, o) {
  ...
  if (o.items) {
    s.addText(
      o.items.map((t, i) => ({ text: t, options: { bullet: true, breakLine: i < o.items.length - 1 } })),
      {
...
function dato(s, x, y, w, numero, etiqueta, tintaNum) {
  s.addText(numero, {
    ...
    fontFace: H, fontSize: 54, bold: true, color: tintaNum || DARK, valign: "bottom",
  });
  s.addText(etiqueta, {
    x: x, y: y + 0.95, w: w, h: 0.6, isTextBox: true, margin: 0,
...
/* 1 - Portada */
{
  ...
  s.addText("DULXELITOS", {
    x: M, y: 2.2, w: CW, h: 1.5, isTextBox: true, margin: 0,
  ...
  s.addText("Trabajo Práctico Final · Parte 1 · Modelo de negocio actual", {
    x: M, y: 3.8, w: CW, h: 0.45, isTextBox: true, margin: 0,
    fontFace: B, fontSize: 20, color: PALE,
  });
  s.addText("Lepez Joaquín · Geyer Juan José · Lopez Juan · Sorato Emiliano · Olivares Manuel", {
...
/* 2 - La empresa */
{
  ...
  const anchoD = (CW - 3 * 0.5) / 4;
  const datos = [
    ["1973", "53 años fabricando snacks", ORANGE],
    ["15-20", "personas, empresa familiar", ORANGE],
    ["3", "líneas: salados, dulces, repostería", DARK],
    ["+50", "tipos de productos", DARK],
  ];
  datos.forEach((d, i) => dato(s, M + i * (anchoD + 0.5), 2.2, anchoD, d[0], d[1], d[2]));
  tarjeta(s, {
    x: M, y: 4.9, w: CW, h: 1.2, fill: T_NEUTRAL, tinta: DARK,
    texto: "Fabrica sus líneas propias y además fracciona y envasa producto de terceros con su marca.",
    pt: 17,
  });
  pie(s);
...
{
  const s = nuevo(DARK);
  titulo(s, "LE VENDE AL DISTRIBUIDOR", CREAM);
  bajada(s, "Nunca al consumidor final", PALE);
  ...
  s.addText("Mendoza concentra el 35% de la red", {
...
/* 4 - Con que la miramos */
{
  ...
  s.addText("CON QUÉ LA MIRAMOS", {
    x: M, y: 0.9, w: CW, h: 0.9, isTextBox: true, margin: 0,
  ...
  s.addText("Tres herramientas de la materia, en este orden", {
    x: M, y: 1.8, w: CW, h: 0.45, isTextBox: true, margin: 0,
  ...
  herr.forEach((h, i) => {
    ...
    s.addShape(pres.ShapeType.roundRect, {
      x: x, y: 2.7, w: anchoH, h: 2.4, fill: { color: CREAM }, rectRadius: 0.1,
    });
    circulo(s, h[0], x + 0.35, 2.95, 0.6, DARK);
    s.addText(h[1], {
      x: x + 0.35, y: 3.8, w: anchoH - 0.7, h: 1.0, isTextBox: true, margin: 0,
      fontFace: H, fontSize: 22, bold: true, color: DARK, valign: "top",
    });
  });
...
/* 5 - FODA */
{
  ...
  tarjeta(s, {
    ...
    items: ["57 distribuidores construidos en 53 años", "Logística propia más transporte tercerizado", "Habilitaciones RNE y RNPA vigentes"],
    pt: 15,
  });
  tarjeta(s, {
    x: M + aw + 0.35, y: 1.75, w: aw, h: ah, fill: T_RED, tinta: RED, titulo: "DEBILIDADES",
    items: ["No conocen el costo real por producto", "Planta saturada, tapada con horas extras", "El saber del negocio está en los dueños"],
    pt: 15,
  });
  tarjeta(s, {
    x: M, y: 4.25, w: aw, h: ah, fill: T_BLUE, tinta: BLUE, titulo: "OPORTUNIDADES",
    items: ["Crecer por fraccionamiento sin ampliar planta", "Aiello como modelo de cadena repetible", "El equipo tiene ganas de capacitarse"],
    pt: 15,
  });
  tarjeta(s, {
    x: M + aw + 0.35, y: 4.25, w: aw, h: ah, fill: T_ORANGE, tinta: "8A5714", titulo: "AMENAZAS",
    items: ["Un solo proveedor de cada insumo", "Sin margen de planta si sube la demanda", "Perder un cliente sin llegar a enterarse"],
    pt: 15,
  });
...
/* 6 - Business Model Canvas, agrupado en cuatro preguntas */
{
  const s = nuevo(CREAM);
  const MC = 0.45;
  const CWC = W - 2 * MC;
  s.addText("BUSINESS MODEL CANVAS", {
    x: MC, y: 0.34, w: CWC, h: 0.52, isTextBox: true, margin: 0,
    fontFace: H, fontSize: 30, bold: true, charSpacing: 1.2, color: DARK, valign: "middle",
  });
  s.addText("Los nueve bloques, agrupados en cuatro preguntas", {
    x: MC, y: 0.86, w: CWC, h: 0.3, isTextBox: true, margin: 0,
  ...
  const yRot1 = 1.24, yGrid = 1.74, hAlto = 3.2, hFila = (hAlto - g) / 2;
  const yRot2 = yGrid + hAlto + 0.13, yBaja = yRot2 + 0.42, hBaja = 1.2;

  // Rotulo de grupo: el nombre, la pregunta y una raya del color del grupo.
  function grupo(x, w, y, nombre, pregunta, tinta) {
    s.addText(
      [
        { text: nombre, options: { bold: true } },
        { text: "   " + pregunta, options: { bold: false } },
      ],
      {
        x: x, y: y, w: w, h: 0.28, isTextBox: true, margin: 0,
        fontFace: H, fontSize: 12, charSpacing: 0.9, color: tinta, valign: "middle",
      }
    );
    s.addShape(pres.ShapeType.rect, { x: x, y: y + 0.3, w: w, h: 0.045, fill: { color: tinta } });
  }

  grupo(col(0), colW * 2 + g, yRot1, "EL CEREBRO", "¿CÓMO?", GREY);
  grupo(col(2), colW, yRot1, "EL CORAZÓN", "¿QUÉ?", ORANGE);
  grupo(col(3), colW * 2 + g, yRot1, "LOS CLIENTES", "¿QUIÉN?", BLUE);
  grupo(MC, CWC, yRot2, "LA ECONOMÍA", "¿CUÁNTO?", RED);

  // Sin vinetas: en celdas angostas la sangria parte todo en dos lineas.
  function mini(x, y, w, h, fill, tit, tinta, lineas, cuerpo, titular) {
    ...
    const filas = lineas.map((t, i) => ({ text: t, options: { breakLine: i < lineas.length - 1 } }));
    if (titular) {
      filas.unshift({ text: titular, options: { bold: true, color: ORANGE, breakLine: true } });
    }
    s.addText(filas, {
      x: x + 0.13, y: y + 0.36, w: w - 0.26, h: h - 0.46, isTextBox: true, margin: 0,
      fontFace: B, fontSize: 10, color: cuerpo || DARK, lineSpacing: 15, valign: "top",
    });
  }

  mini(col(0), yGrid, colW, hAlto, T_NEUTRAL, "8 · SOCIOS", GREY,
    ["Materia prima a granel", "Producto terminado a granel", "Transportistas tercerizados", "Aiello Supermercados"]);
  mini(col(1), yGrid, colW, hFila, T_NEUTRAL, "7 · ACTIVIDADES", GREY,
    ["Producción", "Fraccionamiento y envasado", "Distribución y logística", "Compras", "Administración y cobranza"]);
  mini(col(1), yGrid + hFila + g, colW, hFila, T_NEUTRAL, "6 · RECURSOS", GREY,
    ["La red de +50 distribuidores", "Planta al límite", "Marca y flota propia", "Planilla excel inexacta"]);
  mini(col(2), yGrid, colW, hAlto, DARK, "2 · PROPUESTA", ORANGE,
    ["Logística como diferencial", "Precio competitivo", "Varios formatos por producto", "Fraccionamiento de terceros"], PALE,
    "Rentabilidad al que revende");
  mini(col(3), yGrid, colW, hFila, T_BLUE, "4 · RELACIÓN", BLUE,
    ["La mantiene la empresa", "Exclusividad territorial", "Sin CRM"]);
  mini(col(3), yGrid + hFila + g, colW, hFila, T_BLUE, "3 · CANALES", BLUE,
    ["Flota, fletes y retiro", "Viajante con zona específica", "WhatsApp y teléfono", "Redes: marca", "Sin tienda online"]);
  mini(col(4), yGrid, colW, hAlto, T_BLUE, "1 · SEGMENTOS", BLUE,
    ["57 distribuidores mayoristas", "7 provincias, con exclusividad", "Aiello, única cadena, San Luis", "Nunca al consumidor final"]);
  mini(MC, yBaja, colW * 3 + 2 * g, hBaja, T_RED, "9 · ESTRUCTURA DE COSTOS", RED,
    ["Materia prima · packaging y film (+25 formatos) · mano de obra · logística y flete"]);
  mini(col(3), yBaja, colW * 2 + g, hBaja, T_GREEN, "5 · FUENTES DE INGRESOS", GREEN,
    ["Venta mayorista · Aiello · producto fraccionado"]);

  s.addNotes("El canvas dividido en cuatro preguntas. EL CEREBRO es cómo lo hace: socios, actividades y recursos. EL CORAZÓN, el bloque oscuro del centro, es qué ofrece: rentabilidad al que revende. LOS CLIENTES es a quién le vende: segmentos, relación y canales. Y LA ECONOMÍA, abajo, es cuánto entra y cuánto sale.");
}
...
/* 8 - Conclusiones */
{
  ...
  const puntos = [
    ...
    ["3", "Su activo más valioso no tiene sistema detrás", "+50 distribuidores establecidos, sin CRM y sin medición de cartera."],
  ];
  puntos.forEach((p, i) => {
    const x = M + i * (cw3 + 0.35);
    s.addShape(pres.ShapeType.roundRect, {
      x: x, y: 2.75, w: cw3, h: 3.2, fill: { color: "22353F" }, rectRadius: 0.1,
    });
    circulo(s, p[0], x + 0.32, 3.05, 0.6, ORANGE, DARK);
    s.addText(p[1], {
      x: x + 0.32, y: 3.85, w: cw3 - 0.64, h: 0.9, isTextBox: true, margin: 0,
      fontFace: H, fontSize: 17, bold: true, color: CREAM, valign: "top",
    });
    s.addText(p[2], {
      x: x + 0.32, y: 4.75, w: cw3 - 0.64, h: 1.05, isTextBox: true, margin: 0,
